...
router.post('/', async (req, res) => {
  ...
  if (!trabajador || !fechaInicio || !fechaFin || !valorAPagar) {
    return res.status(400).json({ error: 'Faltan datos requeridos (trabajador, fechas, valorAPagar)' });
  }
  ...
  const templatePath = path.join(process.cwd(), 'public', 'templates', 'desprendible.xlsx');
  ...
  try {
    ...
    res.status(500).json({ error: `Error al generar el Excel o convertir a PDF: ${error.message}` });
  }
});
